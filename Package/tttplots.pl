...
#
	...
	print GPL_LIN_FILE <<EOF;
		...
		plot "$emp_lin_filename" t "empirical" w points, "$the_lin_filename" t "estimated" w lines lt 3, "$up_lin_filename" t "+1 std dev range" w lines lt 4, "$lo_lin_filename" t "-1 std dev range" w lines lt 4 
		quit
EOF
